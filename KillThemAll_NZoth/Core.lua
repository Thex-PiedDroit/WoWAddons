...
function l_events:ADDON_LOADED(sAddonName)
	...
	if sAddonName ~= "KillThemAll_NZoth" then
		return;
	end
	...
	if g_godsListSettings ~= nil then
		if g_ktaCurrentSettings ~= nil and g_ktaCurrentSettings.m_sGods ~= nil then
			...
			if TableContains(currentGods, g_nZoth.m_sDataName) then
				SetGods(currentGods, true, true);
			end
		end

		AddGodToSettingsList(g_nZoth);
	else
		KTA_Print("An update is available for KillThemAll. KillThemAll_NZoth will not work until both are updated.");
		return;
	end
end
