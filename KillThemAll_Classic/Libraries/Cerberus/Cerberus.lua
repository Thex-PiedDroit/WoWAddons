...
local function GetCallingLine(bFromRegistration)
	...
	local iAddonFolderPos = string.find(sCallingLine, "AddOns\\") or string.find(sCallingLine, "dOns\\") or 1;
...
_G["Cerberus_HookThisFile"] = function(bFromRegistration)
	...
	local iCallerStackLevel = (bFromRegistration and 3) or 2;

	if getfenv(iCallerStackLevel) == g_cerberus[sCurrentlyLoadingAddonName] then
	...
	elseif string.find(GetCallingLine(bFromRegistration), "in main chunk") == nil then
		...
		setfenv(iCallerStackLevel, g_cerberus[sCurrentlyLoadingAddonName]);
	end
end
