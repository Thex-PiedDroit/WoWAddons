...
function l_events:ADDON_LOADED(sAddonName)
	...
	if sAddonName ~= "KillThemAll_XalAtath" then
		return;
	end
	...
	if g_godsListSettings ~= nil then
		...
		AddGodToSettingsList(g_xalAtath);
	else
		KTA_Print("An update is available for KillThemAll. KillThemAll_XalAtath will not work until both are updated.");
		return;
	end
end
