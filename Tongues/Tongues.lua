...
Tongues = Class:inherits(Tongues,
{
	...
	{
		Character =
		{					-- Player settings get applied only the the current player
			Language = nil,
			Fluency = {},
			--LD = {},
...
-----=================================
		...
		if Tongues.MountTable[mountType] ~= nil then
			...
			if mountspeaktype ~= nil and mountlanguage ~= nil then
				...
				if string.find(msg, "(.-)(|[cC][0-9a-fA-F]+|H[%a%A]-|h|r)") ~= nil then
					...
					, function(a, link)
						return Tongues:TranslateWord(a, mountlanguage) .. link;
					end);
				end
				...
				if string.find(msg, "(.*)(|[cC][0-9a-fA-F]+|H[%a%A]-|h|r)(.-)$") ~= nil then
					msg = string.gsub(msg, "(.*)(|[cC][0-9a-fA-F]+|H[%a%A]-|h|r)(.-)$"
					, function(a, link, c)
						return a .. link .. Tongues:TranslateWord(c, mountlanguage);
					end);
				else
		...
		if Tongues.PetTable[UnitCreatureFamily("pet")] ~= nil then
			...
			if petspeaktype ~= nil and petlanguage ~= nil then
				...
				if string.find(msg, "(.-)(|[cC][0-9a-fA-F]+|H[%a%A]-|h|r)") ~= nil then
					msg = string.gsub(msg, "(.-)(|[cC][0-9a-fA-F]+|H[%a%A]-|h|r)"
					, function(a, link)
						return Tongues:TranslateWord(a, petlanguage) .. link;
					end);
				end
				---- DO THE LAST PART OF THE MESSAGE IF NOT A LINK
				if string.find(msg, "(.*)(|[cC][0-9a-fA-F]+|H[%a%A]-|h|r)(.-)$") ~= nil then
					msg = string.gsub(msg, "(.*)(|[cC][0-9a-fA-F]+|H[%a%A]-|h|r)(.-)$"
					, function(a, link, c)
						return a .. link .. Tongues:TranslateWord(c, petlanguage);
					end);
				else
				---- IF NO LINK THEN DO ANYWAY
	...
	OnEvent = function(self, event, ...)--onevent functions and set up
		...
		elseif event == "UPDATE_CHAT_COLOR" then
			...
			or typec == "OFFICER"
			or typec == "RAID"
			or typec == "RAID_WARNING"
			or typec == "RAID_LEADER"
			or typec == "PARTY_LEADER"
			or typec == "BATTLEGROUND"
			or typec == "BATTLEGROUND_LEADER"
			or typec == "CHANNEL"
			or typec == "INSTANCE_CHAT"
			or typec == "INSTANCE_CHAT_LEADER" then
	...
	HandleSend = function(self, msg, chatType, langID, language, channel)--HANDLE TEXT SENDING
		...
		local visibleChat = chatType ~= "WHISPER" and chatType ~= "whisper" and chatType ~= "EMOTE" and chatType ~= "emote";

		for kname, vname in pairs(self.Settings.Character.Translators) do

			local id, langNam = GetSpeaking();
			if visibleChatAndSpecialLanguage and langNam ~= "Common" and langNam ~= "Orcish" then
		...
		if (chatType == "WHISPER" or chatType == "whisper") then
			...
			local languageIsDefaultRacial =
				   UnitRace("player") == BRAC["Human"] and self.Settings.Character.Language == T_Common
				...
				or UnitRace("player") == "Pandaran" and self.Settings.Character.Language == "Pandaran";
			...
			if (self.Settings.Character.Screen.Party == true and (chatType == "PARTY" or chatType == "party" or chatType == "PARTY_LEADER")) then
				if languageIsDefaultRacial then
					language = nil;
				end
	...
	HandleReceive = function(self, CHAT_FRAME, event, ...)--HANDLE RECIVING OF TEXT
		...
		or (event == "CHAT_MSG_INSTANCE_CHAT") then
			...
			if ((lang == T_Common) and (UnitFactionGroup("player") == BFAC["Alliance"]))
			or ((lang == T_Orcish) and (UnitFactionGroup("player") == BFAC["Horde"])) then
				...
				if string.match(message, "^%[[%a%s'%-]+%][%a%A]+") then
					...
					if (math.random(1, 100) <= fluency) then
						...
						newMessageDataPack.trans = "RT";
						newMessageDataPack.fluency = fluency;
						newMessageDataPack.channel = event;
						newMessageDataPack.frame = cID;
						newMessageDataPack.language = language;
						print("7");
						Tonguesc:TonguesSendMessage("WHISPER", snder, newMessageDataPack)--TONGUES COME RT : REQUEST TRANSLATE

						dontprint = 0;

					elseif (event~= "CHAT_MSG_RAID_WARNING")
						or (event~= "CHAT_MSG_OFFICER")
						or (event~= "CHAT_MSG_BATTLEGROUND")
						or (event ~= "CHAT_MSG_INSTANCE_CHAT_LEADER")
						or (event ~= "CHAT_MSG_INSTANCE_CHAT") then
			...
			--- If player does know other Blizzard Languages (ie blood elf with Thalassian), but the character /shouldn't/ know it (for RP reasons,) hash it here
				...
				--- If player doesn't know other Blizzard Languages (ie human with gnomish), but the character /should/ know if (for RP reasons), send request
				--- Not sure if Cross Faction can be done this way, but it will be disabled here if it can to meet EULA
					...
					if self:IsMyFactionLanguage(lang) == true then
						...
						if (math.random(1, 100) <= fluency) then
							...
							newMessageDataPack.language = language;
							print("8");
							Tonguesc:TonguesSendMessage("WHISPER", snder, newMessageDataPack);
		...
		elseif (event == "CHAT_MSG_EMOTE") then
			...
			if string.match(message, "^'s pet [%a%A]+ [%a%A]+, \"[%a%A]+\"") then
				...
				if (math.random(1, 100) <= fluency) then
					...
					newMessageDataPack.trans = "PR";
					newMessageDataPack.fluency = fluency;
					newMessageDataPack.companionName = petname;
					newMessageDataPack.frame = cID;
					newMessageDataPack.language = petlanguage;
					newMessageDataPack.companionSpeakType = petspeaktype;
					print("9");
					Tonguesc:TonguesSendMessage("WHISPER", snder, newMessageDataPack);
					dontprint = 0;
				else
			...
			elseif string.match(message, "^'s mount [%a%A]+ [%a%A]+, \"[%a%A]+\"") then
				...
				if (math.random(1, 100) <= fluency) then
					--SendAddonMessage("Tongues", prepstring, "WHISPER", snder);
					newMessageDataPack.trans = "RMN";
					newMessageDataPack.fluency = fluency;
					newMessageDataPack.companionName = mountname;
					newMessageDataPack.frame = cID;
					newMessageDataPack.language = mountlanguage;
					newMessageDataPack.companionSpeakType = mountspeaktype;
					print("10");
					Tonguesc:TonguesSendMessage("WHISPER", snder, newMessageDataPack);
					dontprint = 0;
				else
	...
	LearnRequest = function(self, language,faction,race, class, frame, fluency, toperson)
		if self.Settings.Character.LanguageLearning == true then
			...
			local newMessageDataPack = {};
			newMessageDataPack.trans = "RL";
			newMessageDataPack.language = language;
			newMessageDataPack.faction = faction;
			newMessageDataPack.race = race;
			newMessageDataPack.class = class;
			newMessageDataPack.frame = frame;
			newMessageDataPack.fluency = fluency;
			print("11");
			Tonguesc:TonguesSendMessage("WHISPER", toperson, newMessageDataPack);
		end
	end,
	...
	ApplySpeech = function(self, msg, languageflag)
		...
		if (self[type] ~= nil) and (self[type][s] ~= nil) then
			...
			if languageflag == true then
				if (self:hasStarForm() ~= true and self.Settings.Character.ShapeshiftLanguage == true and GetShapeshiftForm(true) ~= 0 and Tclass == "DRUID")
				and (not string.find(msg, "^[(]")) then
					msg = self:ApplyLanguage("Language", msg);

				elseif (((self.Settings.Character.Language == T_Common) and (self.Settings.Character.Faction ~= "Alliance"))
					and ((self.Settings.Character.Language == T_Orcish) and (self.Settings.Character.Faction ~= "Horde"))
					and (self.Settings.Character.Language ~= self:ReturnSpecialLanguage()))
					and not string.find(msg, "^[(]") then
...
---------------------------------------------------------------------------------------
	ParseLink = function(self, msg, language)
		...
		if string.find(msg, "(.-)(|[cC][0-9a-fA-F]+|H[%a%A]-|h|r)") ~= nil then
			msg = string.gsub(msg, "(.-)(|[cC][0-9a-fA-F]+|H[%a%A]-|h|r)"
			, function(a, link)
				return self:ProcessMessage(a) .. link;
			end);
		end
		---- DO THE LAST PART OF THE MESSAGE IF NOT A LINK
		if string.find(msg, "(.*)(|[cC][0-9a-fA-F]+|H[%a%A]-|h|r)(.-)$") ~= nil then
			msg = string.gsub(msg, "(.*)(|[cC][0-9a-fA-F]+|H[%a%A]-|h|r)(.-)$"
			, function(a, link, c)
				return a .. link .. self:ProcessMessage(c);
			end);
		else
		---- IF NO LINK THEN DO ANYWAY
	...
	ParseLink2 = function(self, msg, language)
		...
		if string.find(msg, "(.-)(|[cC][0-9a-fA-F]+|H[%a%A]-|h|r)") ~= nil then
			msg = string.gsub(msg, "(.-)(|[cC][0-9a-fA-F]+|H[%a%A]-|h|r)"
			, function(a, link)
				return self:TranslateWord(a, language) .. link;
			end);
		end
		---- DO THE LAST PART OF THE MESSAGE IF NOT A LINK
		if string.find(msg, "(.*)(|[cC][0-9a-fA-F]+|H[%a%A]-|h|r)(.-)$") ~= nil then
			msg = string.gsub(msg, "(.*)(|[cC][0-9a-fA-F]+|H[%a%A]-|h|r)(.-)$"
			, function(a, link, c)
				return a .. link .. self:TranslateWord(c, language);
			end);
		else
		---- IF NO LINK THEN DO ANYWAY
	...
	UnderstandPartial = function(self, fluency, lang)
		...
		local msg = self.PreviousSentMsg;
		...
		---- IF NO LINK THEN DO ANYWAY
			if  math.random(1, 100) > fluency
			and math.random(1, 100) > fluency then
	...
	IsMyFactionLanguage = function(self, language)
		...
		if playerFaction == BFAC["Alliance"] then
			if language == T_Common
			or language == T_Gnomish
			or language == T_Dwarvish
			or language == T_Darnassian
			or language == T_Draenei
			--or language == T_Gilnean then
			or langauge == "Pandaren" then
				return true;
			end

		elseif playerFaction == BFAC["Horde"] then
			if language == T_Orcish
			or language == T_Troll
			or language == T_Forsaken
			or language == T_Taurahe
			or language == T_Thalassian
			or langauge == BRAC["Goblin"]
			or langauge == "Pandaren" then
				return true;
			end
		end
